...
export const uploadFile = createAsyncThunk(
  'upload/uploadFile',
  async (
    fileInfo: { video: File; user_id: number; profile_id: number },
    thunkAPI
  ) => {
    try {
      ...
      const response = await fetch(
        `https://metachecker-server.onrender.com/api/upload`,
        {
          method: 'POST',
          body: formData,
        }
...
export const getProfiles = createAsyncThunk(
  ...
  async (profileInfo: { user_id: number }, thunkAPI) => {
    try {
      const response = await fetch(
        `https://metachecker-server.onrender.com/api/user/${profileInfo.user_id}`,
        {
          method: 'GET',
        }
      )
      ...
      .addCase(
        ...
        (
          ...
          action: PayloadAction<{
            QCResults: QCResults
            resJSON: any
            videoProfileInterface: videoProfileInterface
          }>
        ) => {
          ...
          state.probedMetadata = {
            ...
            r_frame_rate:
              action.payload.resJSON.allVideoData.video.r_frame_rate,
          ...
          state.videoProfileInterface = action.payload.videoProfileInterface
        }
      )
